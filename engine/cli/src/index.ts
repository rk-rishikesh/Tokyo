...
import { searchSnapshot, generateContentKey, shortId, checkSubjectAddressing, renderFindings, SOURCE_KINDS, type NamespaceKind, type Resolution, type Source, type SourceKind } from '@recall/core'
import { recoverMessageAddress, type Hex } from 'viem'
import { findOwner } from '@recall/core'
...
async function main(): Promise<void> {
  ...
  switch (cmd) {
    ...
    case 'pull-proposal': {
      ...
      const cids: string[] = []
      if (v.from) {
        const { getContenthash } = await import('@recall/core')
