    ...
    <>
      ...
      <main className="w-full px-5 sm:px-8 lg:px-10">
        ...
        <Section title="Attach it" intro="One npm package, run locally by your agent host. No hosting, no account.">
          <Steps steps={[
            { title: 'Add the server to your agent host', body: <>Claude Code shown; any MCP client takes the same command + env.</>, code: `claude mcp add knowledge -e KNOWLEDGE_AGENT=<your-name.eth> \\\n  -- npx -y @knowledge01/mcp` },
            { title: 'Resolve, then search', body: <>Start with <code>knowledge_resolve</code> to learn version, policy and roles; then search. Public namespaces need no key.</>, code: `knowledge_resolve({ namespace: "cancer-research.eth" })\nknowledge_search({ namespace: "cancer-research.eth", query: "PARP inhibitor ovarian" })\nknowledge_sources({ namespace: "cancer-research.eth", id: "k_9d57e7d6ef72" })` },
